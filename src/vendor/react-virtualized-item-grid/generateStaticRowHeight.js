...
function generateRowHeightWithoutHeaderOrFooter(
  getRowHeight: (input: { index: number }) => number
) {
  return getRowHeight({ index: 0 })
}

function generateRowHeightWithHeader(
  getRowHeight: (input: { index: number }) => number
) {
...
function generateRowHeightWithFooter(
  getRowHeight: (input: { index: number }) => number,
  rowCount: number
) {
...
function generateRowHeightWithHeaderAndFooter(
  getRowHeight: (input: { index: number }) => number,
  rowCount: number
) {
  const headerHeight = getRowHeight({ index: 0 })
